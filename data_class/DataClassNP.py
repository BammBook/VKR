...
def set_Imax_and_t_max(I_A, time):
    ...
    for i in range(1, len(time) - 2, 1):
        if I_A[i] - I_A[i + 1] > 0 and I_A[i - 1] - I_A[i] < 0 and I_A[i] > eps:
            Imax_A.append(I_A[i])
            Imax_A_time.append(time[i])
...
class DataClass:
    def __init__(self, datatype, filepath, filename):
        try:
            if datatype == "csv":
            ...
            self.Imax_A, self.Imax_A_time = set_Imax_and_t_max(self.I_A, self.time)
