...
print(f'length of Imax data: {len(data.Imax_A[:len(data.Imax_A_relative_time_partial)])}')
print(f'Imax data: {data.Imax_A[:len(data.Imax_A_relative_time_partial)]}\n')

print(f'length of Imax time data: {len(data.Imax_A_time[:len(data.Imax_A_relative_time_partial)])}')
print(f'I max time: {data.Imax_A_time[:len(data.Imax_A_relative_time_partial)]}\n')

generic_curve = exp_1(data.Imax_A_relative_time_partial,
                      average_C0_1ph,
                      average_C1_1ph)
...
curve_first_harm_eff = convert_to_named_units_coef * first_harm_effective(coef_A_1ph_cold) * generic_curve


print(f'\nlength of I effective calculated: {len(data.Imax_A_relative_time_partial)}\n')

buffer_time = []
buffer_current = []
for i in range(len(eff_curr.time)):
    if i % 400 == 0 and 0 < eff_curr.time[i] <= data.Imax_A_relative_time_partial[-1] * tau * 1e-3:
        buffer_time.append(round(eff_curr.time[i], 2))
        buffer_current.append(eff_curr.I_A_eff_ph[i])

buffer_time = np.array(buffer_time)
buffer_current = np.array(buffer_current)

print(f'\nlength of I effective shorter: {len(buffer_time)}')
print(f'time from model: {buffer_time}')
